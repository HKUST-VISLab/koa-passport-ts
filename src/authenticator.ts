...
// TODO, init properties in session
...
export class Authenticator {
     ...
     *
     */
    public use(strategy: BaseStrategy);
    public use(name: string, strategy: BaseStrategy);
    public use(name: any, strategy?: BaseStrategy) {
        ...
        return async (ctx, next) => {
            ...
            function allFailed() {
                ...
                if (options.failWithError) {
                    throw new AuthenticationError(http.STATUS_CODES[ctx.status], rstatus);
                }
                // console.log("==================");
                ...
                try {
                    ...
                    switch (res.type) {
                        case ActionType.FAIL: {
                            const { challenge, status } = (res as strategies.FailAction);
                        ...
                        case ActionType.REDIRECT: {
                            const { url, status } = (res as strategies.RedirectAction);
                            ctx.status = status;
                            ctx.redirect(url);
                            return;
                        }
                        case ActionType.SUCCESS: {
                            const { user, info } = (res as strategies.SuccessAction);
                            if (callback) {
